...
function do_ki_only_times(batch_size_create_data,listening_length,data_learn,positions,num_ears ;epochs=2,new_data = 5, print_every = batch_size_create_data//1000, evaluation_batch_size = batch_size_create_data)
    ...
    model = Flux.Chain(#Flux.Dense(num_ears*listening_length=>500,Flux.tanhshrink),
        Flux.Dense(2*num_ears=>10*num_ears,Flux.tanhshrink),
        Flux.Dense(10*num_ears=>20*num_ears,Flux.tanhshrink),
        Flux.Dense(20*num_ears=>10*num_ears,Flux.tanhshrink),
        Flux.Dense(10*num_ears=>5*num_ears,Flux.tanhshrink),
        Flux.Dense(5*num_ears=>3))|>device
    model = Flux.f64(model)
    ...
    for epoch in 1:epochs
        ...
        for index in 1:batch_size_create_data
            ...
            grads = Flux.gradient(model) do m
                result = m(data_learn[index])
                Flux.Losses.mse(result, positions[3*(index-1)+1:3*index])#sqrt(sum((result .- positions[3*(index-1)+1:3*index]).^2)) #+sum(sum.(abs2,Flux.trainables(m)))    # L1 pruning now slower
            end
            # Update the parameters so as to reduce the objective,
            # according the chosen optimisation rule:
            Flux.update!(opt_state, model, grads[1])
...
@time data_learn,positions = only_times_and_dist(create_batch_signals_full_data(batch_size_create_data,listening_length))
#println(data_learn,positions)
...
plot_accuracy(train_acc,test_acc,"test_only_times_dist_NADAM")
