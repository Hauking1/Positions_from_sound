...
function do_ki_CNN(batch_size_create_data,listening_length,batches_per_epoch,num_ears;epochs=2,new_data = 5, print_every = batches_per_epoch//100, evaluation_batch_size = batch_size_create_data)
    ...
    Flux.Conv((1, 100), 1=>8, pad=Flux.SamePad(), Flux.leakyrelu),
    Flux.MaxPool((1,3)),
    # x -> Flux.maxpool(x, (1,3)),

    # Second convolution, operating upon a ...x... image
    Flux.Conv((1, 50), 8=>16, pad=Flux.SamePad(), Flux.leakyrelu),
    Flux.MaxPool((1,3)),
    # x -> Flux.maxpool(x, (1,3)),
    ...
    Flux.Dense(Int(floor(listening_length/27)*16), 2),
...
function flat_do_ki_CNN(batch_size_create_data,listening_length,batches_per_epoch,num_ears;epochs=2,new_data = 5, print_every = batches_per_epoch//100, evaluation_batch_size = batch_size_create_data,load_model=true)
    modelCNN = Flux.Chain(
    # First convolution, operating upon a listeninglengthxnum_ears image
    Flux.Conv((1,100), 1=>8, pad=Flux.SamePad(), Flux.leakyrelu),
    Flux.MaxPool((1,3)),
    ...
    if load_model == true
        model_state = JLD2.load("CNN_single_signal.jld2", "model_state")
    ...
    JLD2.jldsave("CNN_single_signal.jld2"; model_state)
...
batch_size_create_data = 200
batches_per_epoch = 10

@time train_acc,test_acc = flat_do_ki_CNN(batch_size_create_data,listening_length,batches_per_epoch,4;epochs=400,new_data=5,print_every=1,evaluation_batch_size=5)
...
plot_accuracy(train_acc,test_acc,"first_cnn_test_single_signal_2")